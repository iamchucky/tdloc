...
HRESULT CALLBACK ControlSizeDlgProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam);
...
static void Refresh(HWND hWndDlg)
{
	...
	for(i=0; i<8; i++)
	{
		if(pCamera->HasVideoMode(7,i))
		{
			...
			ret = SendMessage(hWndItem,CB_SETITEMDATA,ret,(LPARAM)i);
		}
	}
 ...
 */
static HRESULT CALLBACK ControlSizeDlgProc(
...
)
{
	...
	case WM_INITDIALOG:
		...
		SetWindowLongPtr(hWndDlg,GWLP_USERDATA,(LPARAM)(pCamera));
		...
		case WM_COMMAND:
			...
			case IDC_COMBO_MODE:
				if(HIWORD(wParam) == CBN_SELENDOK)
				{
					...
					lData = SendMessage(hWndItem,CB_GETITEMDATA,lData,0);
