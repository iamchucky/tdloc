 ...
 * \param offset the (absolute) offset to use
 ...
 * and only downstream of a validated SetMode(7,?).  If is not clear what the "right"
...
int C1394CameraControlSize::SetOffset(unsigned long offset)
{
	...
	if(this->m_offset != 0)
	{
		...
		DllTrace(DLL_TRACE_CHECK,"ControlSize::SetOffset: Setting w,h,c to %d,%d,%d\n",
			width,height,code);
...
static int ReadRegisterMap(C1394Camera *pCamera, unsigned long root_offset, struct register_mapping *regs);

/**\brief Read all the static inquiry registers */
