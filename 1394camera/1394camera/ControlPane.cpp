 ...
 * \param nIndex Deprecated, will likely become PCONTROL_PANE_EXTENSION
...
static void cpUpdateFeedback(HWND hWndPanel, BOOL UpdateControl)
{
	...
	if(pPaneExt->pControl->StatusAbsControl())
	{
		...
		if(pPaneExt->flags & PIF_TWO_SLIDERS)
		{
			...
			lRes2 = SendMessage(hWndItem,TBM_GETPOS,0,0);
...
static void cpSetupPanel(HWND hWndPanel)
{
	...
	EnableWindow(hWndItem,pPaneExt->pControl->HasOnePush());
	...
	EnableWindow(hWndItem,pPaneExt->pControl->HasManualMode());
...
LRESULT CALLBACK ControlPaneDlgProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	{
	case WM_INITDIALOG:
		// pPaneExt isn't in GWLP_USERDATA, it's lParam
		...
		SetWindowLongPtr(hWnd,GWLP_USERDATA,(LPARAM)(pPaneExt));
		...
		SetWindowLongPtr(hWnd,GWL_ID,pPaneExt->window_id);
	...
	case WM_COMMAND:
		...
		case IDC_BUT_ADVANCED:
			lRes = DialogBoxParam(pPaneExt->hInstance,MAKEINTRESOURCE(IDD_ADVANCED_CONTROL),hWnd,(DLGPROC)AdvControlDialogProc,(unsigned long)(pPaneExt->pControl));
			pPaneExt->pControl->Status();
			cpSetupPanel(hWnd);
			lRetval = TRUE;
			break;
